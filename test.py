...
parser.add_argument('--mps', default=False, type=str2bool,
                    help='Use mps for inference')
...
def test_net(net, image, text_threshold, link_threshold, low_text, cuda, mps, poly, refine_net=None):
    t0 = time.time()
...
if __name__ == '__main__':
    ...
    if args.cuda:
        ...
        net.load_state_dict(copyStateDict(torch.load(
            args.trained_model, map_location='mps')))
        net = net.to("mps")
    else:
        net.load_state_dict(copyStateDict(torch.load(
    ...
    if args.refine:
        ...
        if args.cuda:
            ...
            refine_net.load_state_dict(copyStateDict(
                torch.load(args.refiner_model, map_location="mps")))
    ...
    for k, image_path in enumerate(image_list):
        ...
        bboxes, polys, score_text = test_net(
            net, image, args.text_threshold, args.link_threshold, args.low_text, args.cuda, args.mps, args.poly, refine_net)
